...
function loadContractArtifact() {
  const artifactPath = path.join(__dirname, '../out/TikTokCampaignVerifier.sol/TikTokCampaignVerifier.json');
...
async function deploy(options: DeployOptions) {
  ...
  const expectedUrl = process.env.EXPECTED_URL || 'https://gist.githubusercontent.com/dpinones/7ddebc14210d404ca6d4951528ff1036/raw/64e6e3c9ab44623903744219034c06eafb8e312b/mockTikTokVideosResponse.json';
  ...
  if (verify) {
    ...
    console.log(`forge verify-contract ${receipt.contractAddress} TikTokCampaignVerifier --chain ${network} --watch`);
