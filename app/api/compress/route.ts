...
export async function POST(request: NextRequest) {
  try {
    ...
    const { presentation, handleTiktok } = body;
    ...
    if (!handleTiktok) {
      return NextResponse.json(
        { error: 'TikTok handle is required for extraction' },
        { status: 400 }
      );
    }

    // Build JMESPath queries to extract TikTok campaign data
    const extractConfig = {
      "response.body": {
        "jmespath": [
          `campaign_id`,
    ...
    console.log('Compressing web proof for TikTok handle:', handleTiktok);
